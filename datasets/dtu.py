...
class DTUDataset(Dataset):
    ...
    def read_center_img(self, filename):
        img = Image.open(filename)
        # scale 0~255 to 0~1
        np_img = np.array(img, dtype=np.float32) / 255.
        return np_img
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            center_img = self.read_center_img(img_filename)
            intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename)

            if self.mode in ["train", "val"]:
                if self.data_scale == "raw":
                    img = self.crop_img(img)
                    center_img = self.crop_img(center_img)
                    intrinsics[:2, :] *= 2.0
                if self.mode == "train" and self.robust_train:
            ...
            center_imgs.append(center_img.transpose(2, 0, 1))

            # reference view
            if i == 0:
                # @Note depth values
                diff = 0.5 if self.mode in ["test", "val"] else 0  # 测试和评估时候，这里为什么要少0.5呢？会不会就是这0.5导致了测试过程中误差偏大那么一点点？
