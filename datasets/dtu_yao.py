...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, img_size=None, ndepths=192, interval_scale=1.06, view_shuffling=True, **kwargs):
        super(MVSDataset, self).__init__()
        self.img_size = img_size if img_size is not None else [512, 640]
        assert self.img_size[0] % 32 == 0 and self.img_size[1] % 32 == 0, 'img_wh must both be multiples of 32!'
        ...
        self.anti_view_noisy = True   # Proposed by PVSNet for robust training
        self.view_shuffle = view_shuffling
        ...
        # self.metas = self.metas[:int(0.5 * len(self.metas))]
    ...
    def read_img(self, filename):
        ...
        # img = img.resize(self.img_size[::-1], Image.BILINEAR)
        # scale 0~255 to 0~1
    ...
    def __getitem__(self, idx):
        ...
        if self.anti_view_noisy:
            num = len(src_views)
            src_train_id = src_views[:(self.nviews - 1) // 2] + src_views[num - (self.nviews - 1) // 2:]
        else:
            src_train_id = src_views[:self.nviews - 1]

        if self.view_shuffle:
            random.shuffle(src_train_id)

        view_ids = [ref_view] + src_train_id
        imgs = []
        mask = None
        depth_ms = None
        depth_values = None
