...
parser.add_argument("--root_dir", type=str, default="/home/lida/lida/2Ddataset/tankandtemples/")
parser.add_argument('--out_dir', default='./outputs/tnt/jiayou_test', help='output dir')
parser.add_argument('--ply_path', type=str, default='outputs/tnt/jiayou_test/dypcd_fusion_plys_geo')
...
tnt_fusion_exps = [
    ...
    # },
    {
        "ply_path": "dypcd_fusion_plys_geo",
    ...
    for ref_view, src_views in pair_data:
        ...
        if args.cam_mode == 'short_range':
            ref_intrinsics, ref_extrinsics = read_camera_parameters(
                os.path.join(out_dir, scene, 'cams_{}/{:0>8}_cam.txt'.format(scene.lower(), ref_view)))
        elif args.cam_mode == 'origin':
            ref_intrinsics, ref_extrinsics = read_camera_parameters(
                os.path.join(out_dir, scene, 'cams/{:0>8}_cam.txt'.format(ref_view)))

        ref_img = read_img(os.path.join(out_dir, scene, 'images/{:0>8}.jpg'.format(ref_view)))
        ...
        if fusion_exp['param_strategy'] == 'mean':
            if ref_view % 50 == 0:
                print("-- thresh: {}".format(confidence.mean()))
            ...
            if conf_thresh == -1:
                photo_mask = confidence > confidence.mean()
                if ref_view % 50 == 0:
                    print("-- thresh: mean() {}".format(confidence.mean()))
            else:
                photo_mask = confidence > conf_thresh
                if ref_view % 50 == 0:
                    print("-- thresh: {}".format(conf_thresh))
        ...
        for src_view in src_views:
            ...
            if args.cam_mode == 'short_range':
                src_intrinsics, src_extrinsics = read_camera_parameters(
                    os.path.join(out_dir, scene, 'cams_{}/{:0>8}_cam.txt'.format(scene.lower(), src_view)))
                ref_depth_max, ref_depth_min = read_depth_parameters(
                    os.path.join(root_dir, split, scene, 'cams/{:0>8}_cam.txt'.format(ref_view)))
            elif args.cam_mode == 'origin':
                src_intrinsics, src_extrinsics = read_camera_parameters(
                    os.path.join(out_dir, scene, 'cams/{:0>8}_cam.txt'.format(src_view)))
                ref_depth_max, ref_depth_min = read_depth_parameters(
                    os.path.join(root_dir, split, scene, 'cams/{:0>8}_cam.txt'.format(ref_view)))
        ...
        if ref_view < 3:
            ...
            save_mask(os.path.join(out_dir, scene, "mask/{:0>8}_photo.jpg".format(ref_view)), photo_mask)
            save_mask(os.path.join(out_dir, scene, "mask/{:0>8}_geo.jpg".format(ref_view)), geo_mask)
            save_mask(os.path.join(out_dir, scene, "mask/{:0>8}_final.jpg".format(ref_view)), final_mask)
