...
parser.add_argument('--testlist', default="./datasets/lists/tnt/intermediate.txt", help='testing scene list')  # test_copy test
...
parser.add_argument('--plydir', default='./outputs/tnt/jiayou_test/pcd_fusion_plys_cl/', help='output dir')
...
parser.add_argument('--conf', type=float, default=0.3, help='prob confidence, for pcd')
parser.add_argument('--thres_view', type=int, default=3, help='threshold of num view, for pcd')
parser.add_argument('--depth_thres', type=float, default=1 / 1300, help='depth_thres for pcd')  # 0.001
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, args):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
    ...
    mask = np.logical_and(dist < args.img_dist_thres, relative_depth_diff < args.depth_thres)
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            geo_mask, depth_reprojected, x2d_src, y2d_src = check_geometric_consistency(ref_depth_est, ref_intrinsics, ref_extrinsics,
                                                                                        src_depth_est, src_intrinsics, src_extrinsics, args)
...
def pcd_filter_worker(scan):
    ...
    if scan in img_dist_thres:
        args.img_dist_thres = img_dist_thres[scan]

    filter_depth(args, pair_folder, scan_folder, out_folder, os.path.join(args.plydir, save_name))
