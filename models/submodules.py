...
'''
Juse used for real depth encoding and other test of DSE moudle
'''
import torch
import torch.nn as nn
import torch.nn.functional as F
from models.dfreqemb import *

# ------------------------------------------------------------------------ #
def init_inverse_range(cur_depth, ndepths, device, dtype, H, W):  # 参数解释：当前深度值，深度假设的个数
...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad():
        proj = torch.matmul(src_proj, torch.inverse(ref_proj))  # KR·Inverse(KR) -> 参考图和源图  xs -> xf投影关系
        rot = proj[:, :3, :3]  # [B,3,3]
        ...
        rot_xyz = torch.matmul(rot, xyz)  # [B, 3, H*W] 行代表x,y,z，列代表每个点
...
def depth_regression(p, depth_hypotheses):
    if depth_hypotheses.dim() <= 2:
        # print("regression dim <= 2")  # 置信度输入的,只有一维向量
        depth_hypotheses = depth_hypotheses.view(*depth_hypotheses.shape, 1, 1)  # 针对置信度计算的
...
class FPNFeature(nn.Module):
    ...
    def __init__(self, base_channels):
        ...
        self.out_channels = [8 * base_channels]  # 24个通道3*8=24
...
class DepthAwareFF(nn.Module):
    def __init__(self, args, feat_dim):
        super(DepthAwareFF, self).__init__()
        self.d_refine = RefineNet()
        self.real_depth = args.real_depth
        self.d_encoder = DepthEncoder(num_bands=args.real_depth)
        self.r_encoder = RayEncoder()
        self.fusion = nn.Sequential(
            Conv2d(feat_dim + 19, 32, 3, 1, padding=1),
            Conv2d(32, feat_dim, 3, 1, padding=1)
        )
    def forward(self, feat, depth_last, rgb, intrins):
        _, _, H, W = feat.shape
        # 深度图refine
        depth_last = self.d_refine(depth_last, rgb)  # b 1 h w  # 这里要不要加入refine呢?
        # 深度图编码
        if self.real_depth is False:
            d_code = self.d_encoder(depth_last, intrins=None)  # b 16 h w
        else:
            d_code = self.d_encoder(depth_last, intrins)  # b 16 h w
        # 射线编码
        r_code = self.r_encoder(intrins, H, W)  # b 3 h w
        # 特征混合
        feat = self.fusion(torch.cat([feat, r_code, d_code], dim=1))
        # feat = self.fusion(torch.cat([feat, d_code], dim=1))
        return feat, depth_last
