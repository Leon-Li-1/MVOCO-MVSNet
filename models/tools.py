...
def get_schedular(optimizer, args):
    ...
    if args.scheduler == "steplr":
        lambda_func = lambda step: 1 / 3 * (1 - step / warmup) + step / warmup if step < warmup \
            else (decay ** (milestones <= step).sum())  # NOQA
        ...
        lambda_func = lambda step: 1 / 3 * (1 - step / warmup) + step / warmup if step < warmup else \
            (min_lr + 0.5 * (max_lr - min_lr) * (1.0 + math.cos((step - warmup) / (T_max - warmup) * math.pi))) / max_lr  # NOQA
...
def compute_reconstr_loss_l0_5(warped, ref, mask, simple=True):
    if simple:
        ...
        grad_loss = smooth_l0_5(warped_dx, ref_dx) + smooth_l0_5(warped_dy, ref_dy)
...
def adjust_w_icc1(epoch_idx, w_icc, max_w_icc):
    if epoch_idx >= 2 - 1:   # 2
        w_icc *= 2
