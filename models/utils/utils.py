...
class WarmupCosineAnnealingLR(torch.optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, warmup_iters, total_iters, eta_min=0, last_epoch=-1):
        """
        按迭代次数实现的带 Warmup 的余弦退火学习率调度器
        :param optimizer: 优化器
        :param warmup_iters: Warmup 阶段的迭代次数
        :param total_iters: 总训练迭代次数
        :param eta_min: 最小学习率
        :param last_epoch: 上一个迭代次数索引
        """
        self.warmup_iters = warmup_iters
        self.total_iters = total_iters
        self.eta_min = eta_min
        # 检查 last_epoch 是否合理，不能超过 total_iters
        if last_epoch >= total_iters:
            raise ValueError("last_epoch must be less than total_iters")
