...
import os, sys, time, gc, datetime, logging, json, errno  # NOQA
...
from models.loss import MVSLoss
...
def train(model, model_loss, optimizer, TrainImgLoader, TestImgLoader, start_epoch, args):
    if args.lr_scheduler == 'MS':
        ...
        lr_scheduler = WarmupCosineAnnealingLR(optimizer, warmup_iters=500, total_iters=int(args.epochs * len(TrainImgLoader)), eta_min=1e-5, last_epoch=len(TrainImgLoader) * start_epoch - 1)
    elif args.lr_scheduler == 'onecycle':
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, total_steps=int(args.epochs * len(TrainImgLoader)), last_epoch=len(TrainImgLoader) * start_epoch - 1)
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs, image_outputs = train_sample(model, model_loss, optimizer, sample, epoch_idx, args)
            lr_scheduler.step()
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    logger.info("Epoch {}/{}, Iter {}/{}, 2mm_err={:.3f} | lr={:.6f}, train_loss={:.3f}, abs_depth_err={:.3f}, depth_loss={:.3f}, total_entropy={:.3f}, stage4_entropy={:.3f}, time={:.3f}".format(
                        ...
                        scalar_outputs["total_entropy"],
                        scalar_outputs["stage4_entropy"],
                        time.time() - start_time))
                    # logger.info("Epoch {}/{}, Iter {}/{}, 2mm_err={:.3f} | lr={:.6f}, train_loss={:.3f}, abs_depth_err={:.3f}, s3_pw_loss={:.3f}, s3_dds_loss={:.3f}, time={:.3f}".format(
                    #     epoch_idx, args.epochs, batch_idx, len(TrainImgLoader),
                    #     scalar_outputs["thres2mm_error"],
                    #     optimizer.param_groups[0]["lr"],
                    #     loss,
        ...
        if (epoch_idx % args.eval_freq == 0) or (epoch_idx == args.epochs - 1):
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                global_step = len(TrainImgLoader) * epoch_idx + batch_idx
                # do_summary = global_step % args.summary_freq == 0
                do_summary = global_step % 77 == 0
                loss, scalar_outputs, image_outputs = test_sample_depth(model, model_loss, sample, epoch_idx, args)
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        if not args.notensorboard:
                        ...
                        logger.info(
                            "Epoch {}/{}, Iter {}/{}, 2mm_err={:.3f} | lr={:.6f}, test_loss={:.3f}, abs_err={:.3f}, depth_loss={:.3f}, total_entropy={:.3f}, stage4_entropy={:.3f}, time={:.3f}".format(
...
def train_sample(model, model_loss, optimizer, sample, epoch_idx, args):
    model.train()
    optimizer.zero_grad()
    ...
    # @Note MVSNet main
    outputs = model(sample_cuda, "train")
    # loss, epe, depth_loss, total_entropy, entro_loss = model_loss(
    #     outputs, depth_gt_ms, mask_ms,
    #     stage_lw=[float(e) for e in args.stage_lw.split(",") if e] if (args.stage_lw is not None) else None,
    #     dstage_lw=[float(e) for e in args.dstage_lw.split(",") if e] if (args.dstage_lw is not None) else None,
    #     depth_values=sample_cuda["depth_values"],
    # )
    loss, scale = model_loss(sample_cuda, outputs)
    ...
    scalar_outputs = {
        "loss": loss,
        "epe": scale["epe"],
        "depth_loss": scale["depth_loss"],
        "total_entropy": scale["total_entropy"],
        "stage4_entropy": scale["entro_loss"],
...
@make_nograd_func
def test_sample_depth(model, model_loss, sample, epoch_idx, args):
    if is_distributed:
    ...
    outputs = model_eval(sample_cuda, "val")
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        assert args.loadckpt is not None
    ...
    model_loss = MVSLoss(args)

    # optimizer
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, betas=(0.9, 0.999), weight_decay=args.wd)
    # optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, betas=(0.9, 0.999), weight_decay=args.wd)

    # load parameters
    start_epoch = 0
    if args.resume is True and args.start_ckpts is None:
        saved_models = [fn for fn in os.listdir(args.logdir) if fn.endswith(".ckpt")]
        saved_models = sorted(saved_models, key=lambda x: int(x.split('_')[-1].split('.')[0]))
        loadckpt = os.path.join(args.logdir, saved_models[-1])
        logger.info("resuming: " + loadckpt)
        state_dict = torch.load(loadckpt, map_location=torch.device("cpu"), weights_only=True)
        model.load_state_dict(state_dict['model'])
        optimizer.load_state_dict(state_dict['optimizer'])
        start_epoch = state_dict['epoch'] + 1

    if args.resume is False and args.start_ckpts is not None:

        if not os.path.exists(args.start_ckpts):
            raise NotImplementedError('no checkpoint file from path %s...' % args.start_ckpts)
        logger.info(f'Loading weights from {args.start_ckpts}...')

        # load state dict
        state_dict = torch.load(args.start_ckpts, map_location='cpu', weights_only=True)
    ...
    if args.resume is True and args.start_ckpts is not None:
        raise ValueError(
    ...
    if args.which_dataset == "dtu":
        ...
        test_dataset = DTUDataset(args.testpath, args.testlist, "val", args.n_views, data_scale=args.data_scale)  # val 替换成test
